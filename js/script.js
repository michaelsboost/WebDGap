          ...
          return function(e) {
            ...
            $(".export-as-lin-app").on("click", function() {
              JSZipUtils.getBinaryContent("YourLinApp.zip", function(err, data) {
                ...
                var zip = new JSZip();
                var appName = zip.folder( $("[data-action=sitename]").val().replace(/ /g, "-")  );
                appName.load(data);

                appName.folder("resources/default_app/").load(webAppZipBinary);
                var Img16 = c16[0].toDataURL("image/png");
                var Img32 = c32[0].toDataURL("image/png");
                var Img64 = c64[0].toDataURL("image/png");
                var Img128 = canvas[0].toDataURL("image/png");
                ...
                appName.file($("[data-action=sitename]").val().split(" ").join("-") +".desktop", "[Desktop Entry]\nName="+ $("[data-action=sitename]").val() +"\nPath=/home/kodeweave/"+ $("[data-action=sitename]").val().split(" ").join("-") +"\nActions=sudo;\nExec=./"+ $("[data-action=sitename]").val().split(" ").join("-") +"/electron\nIcon=/home/kodeweave/"+ $("[data-action=sitename]").val().split(" ").join("-") +"/resources/default_app/icons/128.png\nTerminal=true\nType=Application\nStartupNotify=true\n\n");
                appName.file("resources/default_app/package.json", "{\n  \"name\": \""+ $("[data-action=sitename]").val() +"\",\n  \"productName\": \""+ $("[data-action=sitename]").val() +"\",\n  \"version\": \"1.0.0\",\n  \"main\": \"default_app.js\",\n  \"license\": \"MIT\"\n}\n");
                zip.file("make.sh", "if [ -d ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +" ]; then\n  typeset LP_FILE=${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +".desktop\n\n  # Remove the target file if any\n  rm -f ${LP_FILE}\n  printf \"%s[Desktop Entry]\\nName="+ $("[data-action=sitename]").val() +"\\nPath=${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"\\nActions=sudo\\nExec=./"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"/electron\\nIcon=${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"/resources/default_app/icons/128.png\\nTerminal=true\\nType=Application\\nStartupNotify=true > ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +".desktop\" >> ${LP_FILE}\n\n  echo 'Your application and launcher are now located at ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"'\n  rm README.md\n  rm make.sh\n  cd ../\n  rmdir "+ $("[data-action=sitename]").val().replace(/ /g, "-") +"-lin\n  cd ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"/\n  chmod 775 electron\nfi\n\nif [ ! -d ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +" ]; then\n  mv "+ $("[data-action=sitename]").val().replace(/ /g, "-") +" ${HOME}\n\n  typeset LP_FILE=${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +".desktop\n\n  # Remove the target file if any\n  rm -f ${LP_FILE}\n  printf \"%s[Desktop Entry]\\nName="+ $("[data-action=sitename]").val() +"\\nPath=${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"\\nActions=sudo\\nExec=./"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"/electron\\nIcon=${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"/resources/default_app/icons/128.png\\nTerminal=true\\nType=Application\\nStartupNotify=true > ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +".desktop\" >> ${LP_FILE}\n\n  echo 'Your application and launcher are now located at ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"'\n  rm README.md\n  rm make.sh\n  cd ../\n  rmdir "+ $("[data-action=sitename]").val().replace(/ /g, "-") +"-lin\n  cd ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +"/\n  chmod 775 electron\nfi\n\n# For Windows OS\n#if EXIST ${HOME}/"+ $("[data-action=sitename]").val().replace(/ /g, "-") +" (\n  #echo Yes\n#) ELSE (\n  #echo No\n#)\n");
...
$("[data-action=sitename]").val().replace(/ /g, "-") +"-lin.zip` folder anywhere on your computer except the home folder. \n 2. Open a terminal and then navigate to "+ 
$("[data-action=sitename]").val().replace(/ /g, "-") +"'s directory and `run the make.sh file`.\n\n  **example**:\n  cd Downloads/"+ 
$("[data-action=sitename]").val().replace(/ /g, "-") +"-lin\n\n 3. This will move the "+ 
$("[data-action=sitename]").val().replace(/ /g, "-") +" sibling folder and it's decendants to your home directory and create an application launcher.\n");
                var content = zip.generate({type:"blob"});
